...
class IsaacROSRobotSegmentorTest(IsaacROSBaseTest):
    ...
    def test_robot_segmenter(self):
        ...
        try:
            ...
            if WRITE_IMAGES:
                cv2.imwrite(
                    str(TEST_FOLDER) + '/robot_segmenter_output_mask_uint16.png',
                    robot_mask_output)
                cv2.imwrite(
                    str(TEST_FOLDER) + '/robot_segmenter_output_depth_uint16.png',
                    depth_colored)

            # Make sure that the output is the same as cached depth and mask.
            cached_depth_path = str(TEST_FOLDER) + '/golden_robot_segmenter_depth_uint16.npy'
            cached_depth_data = np.load(cached_depth_path).astype(np.uint16)
            cached_mask_path = str(TEST_FOLDER) + '/golden_robot_segmenter_mask_uint16.npy'
            cached_mask_data = np.load(cached_mask_path).astype(np.uint16)
